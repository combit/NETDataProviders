            ...
            try
            {
                ...
                for (int pass = 0; pass < 2; pass++)
                {
                    ...
                    foreach (DataRow dr in currentTable.Rows)
                    {
                        ...
                        if (pass == 1)
                        {
                            // check if this is a view and the views table contains the current table
                            // see #61502, sometimes a view is contained in both schema tables
                            if (dr["TABLE_TYPE"].ToString() == "VIEW")
                            {
                                if (passedTables.Contains(name))
                                    continue;
                            }
                        }
